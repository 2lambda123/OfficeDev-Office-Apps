...
self.addEventListener('install', function(event) {
  ...
  event.waitUntil(
      caches.open(NECESSARY_FILES)
          .then(cache => { return cache.addAll(NECESSARY_FILES_URLS); })
  ...
  return caches.keys()
      ...
      .then(cachesToDelete => {
        return Promise.all(cachesToDelete.map(
            cacheToDelete => { return caches.delete(cacheToDelete); }));
      });
}
...
self.addEventListener(
    'activate',
    function(event) { return removeOldCaches().then(self.clients.claim()); });
...
function fetchAndCache(request, requestURL, pageName) {
  ...
  if (request.destination == "image") {
    return fetch(request).catch(response => { return response; });
  }
    ...
    return fetch(request)
        ...
        .catch(response => {
                   ...
                   return clients.matchAll().then(clientList => {
                     clientList.forEach(
                         client => { client.postMessage("pullingFromCache"); });
...
function fetchAndCacheNoRequest(url) { return fetchAndCache(url, url, null); }
...
function cacheOtherPageLinks(pageName) {
  return checkCache(WAPI_LINKS_URL + pageName + "&prop=links")
      .then(response => { return responseToJSON(response); })
      .then(
          linkList => { return precacheLinks(linkList.parse.links, pageName); })
}
  ...
  return checkCache(WAPI_TOC_URL + pageName)
      ...
      .then(contents => {
        ...
        return Promise.all(contents.mobileview.sections.map(prop => {
          ...
          // can navigate to
          if (prop.toclevel == 1) {
            return checkCache(WAPI_SECTION_URL + pageName +
                              "&section=" + prop.id);
          }
          return Promise.resolve();
        }));
  ...
  return caches.has(pageName).then(hasCache => {
    if (!hasCache) {
      return cachePageSections(pageName)
          .then(() => { return removeOldCaches(pageName); })
          .then(() => { return cacheOtherPageLinks(pageName); })
        ...
        caches
            ...
            .then(cachedResponse => {
              if (cachedResponse) {
                ...
                event.waitUntil(
                    updateCache(event.request, requestURL, pageName));
